...
function useIsAbled({ watch, errors, modalState }: Props) {
    const [isAbled, setIsAbled] = useState<boolean>(false);
    const handleIsAbled = () => {
        if (modalState === 1) {
            if (watch("email")?.length && watch("password")?.length && !errors.email?.message) setIsAbled(true);
